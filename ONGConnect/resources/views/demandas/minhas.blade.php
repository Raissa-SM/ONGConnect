...
<div class="max-w-5xl mx-auto px-6 py-12">
    ...
    <div class="flex items-center justify-between mb-8 gap-4 flex-wrap">
        <div>
            <h1 class="text-3xl font-bold tracking-tight text-ink">Minhas demandas</h1>
            <p class="text-ink-2 text-base mt-1">Gerencie as vagas de voluntariado da sua ONG</p>
        </div>
    ...
    @if($demandas->isEmpty())
        <div class="bg-surface rounded-2xl border border-border/60 p-16 text-center">
            <p class="text-xl font-semibold text-ink mb-2">Você ainda não criou nenhuma demanda</p>
            <p class="text-base text-ink-2 mb-6">Crie sua primeira vaga para começar a receber voluntários.</p>
            <a href="{{ route('demandas.criar') }}"
                    ...
                    <div class="p-5">
                        <div class="flex items-start justify-between gap-4 flex-wrap">
                            <div class="min-w-0 flex-1">
                                ...
                                <div class="flex items-center gap-x-4 gap-y-1 text-sm text-ink-2 flex-wrap">
                            ...
                            <div class="flex flex-wrap gap-2 shrink-0">
                                ...
                                <a href="{{ route('demandas.editar', $demanda->id) }}"
                                   class="text-sm border border-border hover:border-ink-2 text-ink-2 px-4 py-2 rounded-full transition-colors font-medium">
                                    Editar
                                </a>
                                ...
                                @elseif($demanda->status->value === 'aberta')
                                    <form method="POST" action="{{ route('demandas.concluir', $demanda->id) }}"
                                          onsubmit="return confirm('Concluir a demanda? Todas as inscrições aceitas serão marcadas como concluídas. Isso não pode ser desfeito.')">
                                        @csrf
                                        <button type="submit"
                                            class="text-sm bg-primary/10 hover:bg-primary/20 text-primary px-4 py-2 rounded-full transition-colors font-semibold">
                                            Concluir demanda
                                ...
                                <form method="POST" action="{{ route('demandas.destroy', $demanda->id) }}">
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                        onclick="return confirm('Excluir esta demanda? Isso não pode ser desfeito.')"
                                        class="text-sm text-ink-2 hover:text-danger px-4 py-2 rounded-full transition-colors font-medium">
                                        Excluir
                                    </button>
                                </form>
                            </div>
